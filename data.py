...
def calculate_edge_density(img_array):
    """Calculate edge density using gradient-based method"""
    ...
    # Calculate gradients (simple edge detection)
...
def analyze_image_features(img):
    """
    Analyze image features using PIL and numpy only
    """
    ...
    # Ensure RGB format
    if len(img_array_resized.shape) == 3 and img_array_resized.shape[2] == 3:
        rgb_array = img_array_resized
    else:
        rgb_array = np.stack([img_array_resized] * 3, axis=2) if len(img_array_resized.shape) == 2 else img_array_resized[:,:,:3]
    ...
    std_rgb = np.std(rgb_array, axis=(0, 1))
    
    # Convert to HSV for additional analysis
    mean_hsv = rgb_to_hsv(mean_rgb[0], mean_rgb[1], mean_rgb[2])
    
    # Calculate brightness (grayscale)
    brightness = np.mean(np.dot(rgb_array, [0.2989, 0.5870, 0.1140]))
    
    # Calculate edge density
    edge_density = calculate_edge_density(rgb_array)
    ...
    features['mean_saturation'] = mean_hsv[1]
    features['mean_value'] = mean_hsv[2]
...
def classify_image(img):
    ...
    if mean_saturation > 30 and mean_saturation < 100:  # Moderate saturation
        desert_score += 0.2
    if mean_rgb[0] > 120 and mean_rgb[2] > 100:  # Reddish-brown
        desert_score += 0.1
...
if page == "Image Classification":
    ...
    if uploaded_file is not None:
        ...
        with col2:
            ...
            <div class="feature-box">
                ...
                <p><strong>Saturation:</strong> {features['mean_saturation']:.1f}%</p>
        ...
        with metric_col2:
            st.metric("Saturation", f"{features['mean_saturation']:.0f}%",
                     delta=f"{features['mean_saturation']:.1f}%")
    ...
    This satellite image classifier uses computer vision techniques and rule-based classification using only PIL and NumPy.
    ...
    - **Texture Analysis**: Gradient-based edge detection and density calculation
    - **Rule-Based Classification**: Logic-based decision making
    
    ### Key Features:
    - **No OpenCV Required**: Uses PIL and NumPy for all image processing
    - **Lightweight**: Minimal dependencies
    - **Fast Processing**: Efficient numpy operations
    - **Interpretable**: Clear feature analysis and decision rules
    ...
    with st.expander("🏜️ Desert Images"):
        ...
        **Scoring System:**
        ...
        - +0.1 points for reddish-brown colors
        """)
    ...
    - **Image Processing**: PIL for image manipulation and resizing
    - **Feature Analysis**: NumPy for numerical computations
    - **Color Space Conversion**: Custom RGB to HSV conversion
    - **Edge Detection**: Gradient-based approach using numpy
    ...
    with tab2:
        ...
        with col2:
            ...
            **Scoring System:**
            ...
            - Reddish-brown colors → +0.1 points
    ...
    if uploaded_file is not None:
        ...
        with col2:
            ...
            st.write("🎨 **Hue:**")
            st.progress(features['mean_hue'] / 360)
            st.write(f"Value: {features['mean_hue']:.1f}°")
            
            st.write("💫 **Saturation:**")
            st.progress(features['mean_saturation'] / 100)
            st.write(f"Value: {features['mean_saturation']:.1f}%")
            
            st.write("☀️ **Value/Brightness:**")
            st.progress(features['mean_value'] / 100)
            st.write(f"Value: {features['mean_value']:.1f}%")
